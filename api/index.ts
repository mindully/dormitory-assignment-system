...
import type { Student, Room, AssignmentResult, FailedAssignment, AssignmentDashboardStats } from '../src/types';
    ...
    try {
      ...
      function pushAssignment(student: Student, room: Room, groupStudents: Student[]) {
        ...
        assignedList.push({
          ...
          movein_date: student.movein_date, room_no: room.room_no, dorm_type: room.type,
          roommate_id_requested: student.roommate_id, roommate_matched_id: otherGroupMembers || undefined
        });
      }
      function pushFailed(student: Student, reason: string) {
        failedList.push({
          student_id: student.id, student_name: student.name,
          gender: student.gender === 'M' ? '남' : student.gender === 'F' ? '여' : student.gender,
          dorm_pref: student.dorm_pref, movein_date: student.movein_date, roommate_id: student.roommate_id, reason
        });
      }
      ...
      rawGroups.forEach(group => {
        ...
        if (matchedRooms.length === 0) {
          group.students.forEach(s => pushFailed(s, '성별 방 불일치 또는 미보유 기숙사 유형')); return;
        }
        let assignedRoom: Room | null = null;
        for (const r of matchedRooms) {
          if (r.capacity - (roomOccupancy.get(r.room_no) || 0) >= groupSize) { assignedRoom = r; break; }
        }
        if (assignedRoom) {
      ...
      for (const k of Object.keys(firstStudentRaw)) studentColPreview[k] = String(firstStudentRaw[k]).substring(0, 20);
      const roomColPreview: Record<string, string> = {};
      for (const k of Object.keys(firstRoomRaw)) roomColPreview[k] = String(firstRoomRaw[k]).substring(0, 20);
      ...
      return res.json({
        ...
        diagnostics: { student_parsed: studentMatched, room_parsed: roomMatched, student_raw_preview: studentColPreview, room_raw_preview: roomColPreview, raw_student_columns: Object.keys(firstStudentRaw).map(k => JSON.stringify(k)), raw_room_columns: Object.keys(firstRoomRaw).map(k => JSON.stringify(k)), warnings, all_columns_matched: studentsWithoutGender.length === 0 && studentsWithoutDormPref.length === 0 && roomsWithoutGenderType.length === 0 }
      });
    } catch (err: any) {
